  ...
  try {
    ...
    if (type !== accessTokenKey) {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .json(
          genResponse(null, "provided token is not a accessToken", false, null),
        );
    }
    ...
    if (err instanceof jwt.TokenExpiredError) {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .json(genResponse(null, "provided token is expired", false, null));
    }

    if (err instanceof jwt.JsonWebTokenError) {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .json(genResponse(null, "provided token is invalid", false, null));
    }
  }
}
