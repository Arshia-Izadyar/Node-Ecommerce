...
async function createProvider(req, res) {
    ...
    const provider = await Provider.create({name: name, slug: slug, description: description})
    return res.status(StatusCodes.CREATED).json(response(provider, null, true, null))
}
...
async function updateProvider(req, res) {
    ...
    await provider.save()
    return res.status(StatusCodes.ACCEPTED).json(response(provider, null, true, null))
}
...
async function getAllProviders(req, res) {
    ...
    if (sort) {
        order.push(["name", sort.toLocaleLowerCase() === 'desc' ? 'DESC' : 'ASC'])
    }
    ...
    return res.status(StatusCodes.OK).json(response(providers, null, true, {next: next_page_url}))
}


module.exports = {getAllProviders, createProvider, updateProvider, deleteProvider, getProvider}