...
async function createProduct(req, res) {
    ...
    let productImages = null;
        ...
        return res.status(StatusCodes.CREATED).json(response(product, null, true, null))
}
...
async function addProvider(req, res) {
    ...
    if (!providers.length) {
        return res.status(StatusCodes.NOT_FOUND).json(response(null, 'providers not found', false, null))
    }
...
async function deleteProduct(req, res) {
    ...
    if (images.length >= 1) {
        for (let i = 0; i < images.length; i++){
            ...
            let file = path.resolve(__dirname, '../public/uploads', imageFilePath[imageFilePath.length - 1])
...
async function getAllProducts(req, res) {
    ...
    if (name) {
        ...
        order.push(['price', price.toLocaleLowerCase() === 'desc' ? 'DESC' : 'ASC'])
    }
    if (q) {
...
module.exports = {createProduct, getOneProduct, addProvider, removeProvider, deleteProduct, updateProduct, getAllProducts}