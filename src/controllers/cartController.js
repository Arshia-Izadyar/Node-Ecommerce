...
async function getBasket(req) {
  ...
  if (user) {
    shoppingCart = await Cart.findOne({
      where: { userId: user.id, inPayment: false },
      include: [
        {
          model: CartLines,
          as: "cartLines",
      ...
      if (shoppingCart.uuid !== cart) {
        ...
        if (cookieShoppingCart && cookieShoppingCart.cartLines) {
          for (let cookieCartLine of cookieShoppingCart.cartLines) {
            let existingCartLine = shoppingCart.cartLines.find(
              (cartLine) => cartLine.productId === cookieCartLine.productId,
            );
            if (existingCartLine) {
    ...
    shoppingCart = await Cart.findOne({
      where: { uuid: cart, inPayment: false },
      include: [{ model: CartLines, as: "cartLines" }],
    });
  }
...
async function getCart(req, res) {
  ...
  if (shoppingCart.cartLines) {
    for (let i = 0; shoppingCart.cartLines.length > i; i++) {
      ...
      total +=
        shoppingCart.cartLines[i].product.price *
        shoppingCart.cartLines[i].quantity;
    }
  }

  res.cookie("cart", shoppingCart.uuid, {
    signed: true,
    maxAge: 72 * 60 * 60 * 1000,
  }); // 3 days in ms
  return res
    .status(StatusCodes.OK)
    .json(genResponse(shoppingCart, null, true, { total: total }));
}
  ...
  if (quantity > requestedProduct.quantity) {
    return res
      ...
      .json(
        genResponse(
          null,
          `cant request ${quantity} from product ${requestedProduct.name} with quantity ${requestedProduct.quantity}`,
          false,
          null,
        ),
      );
  ...
  if (shoppingCart.cartLines) {
    for (let i = 0; i < shoppingCart.cartLines.length; i++) {
      if (shoppingCart.cartLines[i].productId === productId) {
        ...
        return res
          .status(StatusCodes.OK)
          .json(genResponse(shoppingCart, null, true, null));
      }
    }
  }
  await CartLines.create({
...
async function removeFromCart(req, res) {
        ...
        if (shoppingCart.cartLines[i].quantity > quantity) {
          ...
          return res
            ...
            .json(
              genResponse(
                null,
                `cant remove ${quantity} from ${shoppingCart.cartLines[i].quantity}`,
                true,
                null,
              ),
            );
  ...
  return res
    ...
    .json(
      genResponse(
        null,
        "cart empty or requested product already removed",
        true,
        null,
      ),
    );
